...
@Service
public class CsvHouseholdInfoExtractorService implements HouseholdInfoExtractorService<String> {
    /**
     * @param people input data in {@link Person} records
     * @return CSV String with Household info in one line (number of occupants and address) and in next lines:
    ...
    private String prepareAdultsInfo(List<Person> people) {
        return people.stream()
                ...
                .map(person -> MessageFormat.format("{0}, {1}, {2}", person.firstName(), person.lastName(), person.age()))
                .collect(Collectors.joining("\n"));
    }
}
